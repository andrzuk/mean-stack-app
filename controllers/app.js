var app = angular.module('mainApp', ['authModule', 'pagesModule', 'usersModule', 'messagesModule', 'imagesModule', 'settingsModule', 'visitorsModule', 'loginsModule', 'backupModule', 'todosModule', 'ngSanitize'])
...
.controller('mainController', ['$rootScope', '$scope', '$http', '$document', '$sce', function ($rootScope, $scope, $http, $document, $sce) {
	...
	$http.get('https://ipinfo.io/json').then(function (response) {
		$rootScope.currentIp.ip = response.data.ip;
		$rootScope.currentIp.host = response.data.hostname + ' ' + response.data.city + ' ' + response.data.country;
		$scope.isIpReady = true;
	});
	...
	$scope.getAppLogins = function() {
		$scope.isUserLoggedIn(function(result) {
			if (result) {
				$rootScope.module = 'logins';
				$rootScope.action = 'list';
				$scope.status = null;
				setTimeout(function() {
					$('button#get-logins').click();
				}, 500);
			}
			else {
				$scope.getLogin();
	...
	$scope.registerVisitor = function(hash) {
		...
		var details = {
			ip: $rootScope.currentIp.ip,
			host: $rootScope.currentIp.host,
			referer: $document.getReferer(),
			url: '/' + hash,
		};
		if ($scope.isIpReady) {
			$.each($scope.registerStack, function(index, item) {
				var details = {
					ip: $rootScope.currentIp.ip,
					host: $rootScope.currentIp.host,
					referer: $document.getReferer(),
			...
			if ($scope.registerStack.length) {
				$scope.registerStack = [];
			}
			$http.post('/visitor', details, null).then(function() {});
		}
		else {
			$scope.registerStack.push(hash);
		}
	};
